...
### Reducción del proyecto a 10 – 11 horas

Si el tiempo disponible es menor, se pueden aplicar los siguientes ajustes sin perder el aprendizaje central:

- **Fase 4:** reducir a 5 pruebas documentadas (en lugar de 10) y eliminar la prueba de usuario real.
- **Fase 3:** entregar el archivo HTML ya personalizado con una materia de ejemplo para que los equipos solo ajusten el system prompt.
- **Fase 5:** eliminar la reflexión escrita individual y hacerla oral durante la sesión de preguntas.

### Sobre las API keys

Todos los proveedores ofrecen acceso gratuito o créditos de prueba al registrarse:

- **Claude (Anthropic):** capa gratuita disponible en claude.ai y API con créditos de prueba en console.anthropic.com.
- **OpenAI:** créditos de prueba al crear cuenta en platform.openai.com. GPT-4o-mini es el modelo más económico.
- **Gemini:** completamente gratuito con cuenta de Google en aistudio.google.com.

Se recomienda que el docente obtenga y pruebe las keys con anticipación y tenga al menos una de respaldo para equipos con problemas de registro.

### Privacidad y seguridad de las API keys

Es importante enfatizar a los estudiantes:

- Nunca compartir el archivo HTML con la API key incluida (redes sociales, correo, repositorios públicos).
- Las API keys son credenciales personales: si se filtran, terceros pueden usarlas y generar costos.
- Al terminar el proyecto, revocar la key en el panel del proveedor si ya no se usará.

Este es un punto de aprendizaje valioso sobre seguridad en software, no solo una advertencia administrativa.

### El proyecto como introducción a conceptos de IA

Al presentar y evaluar, se recomienda que el docente guíe una discusión sobre los siguientes conceptos que emergen naturalmente del proyecto:

- **Context window:** por qué el bot "olvida" conversaciones muy largas.
- **Prompt injection:** qué pasa si alguien escribe en el chat instrucciones para que el bot ignore su system prompt.
- **Hallucination:** cuando el modelo inventa información que no tiene (fácil de observar en preguntas específicas de fechas o datos).
- **Temperature y creatividad:** por qué dos preguntas idénticas pueden dar respuestas distintas.
- **Diferencias entre modelos:** comparar el comportamiento del mismo prompt en Claude, GPT y Gemini es en sí mismo un ejercicio valioso.
